...
$(document).ready(function() {
    axios({
        method: 'get',
        url: 'http://api.openweathermap.org/data/2.5/weather?q=london&units=metric&APPID=b0c25191a55587d9b4dc04a46fab2436'
    })
    .then(response => {
    ...
    $('body').append(`<p> ${weatherOutside} ${response.data.main.temp} C </p>`)

    $('body').append(`<p> ${moment().format('LT')}  </p>`)

    // Adding Classes to the tags created in jQuery
    $('p').first().addClass( "temperature")
    $('p').last().addClass( "moment")
    })
    .catch(error => {
        console.log(error)
    })
